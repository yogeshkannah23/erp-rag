"""
Pure-domain service: extracts structured sections from PDF text using regex.
No external dependencies — safe to live in the domain layer.
"""
import re
...
class TextParser:
    ...
    def extract_section(
        ...
        if next_section_name and next_section_name in self.SECTION_PATTERNS:
            next_match = re.search(
                self.SECTION_PATTERNS[next_section_name],
                text[start_pos:],
    ...
    def parse_project_document(self, text: str) -> Dict[str, str]:
        ...
        return {
            name: self.extract_section(
                text, name, section_order[i + 1] if i + 1 < len(section_order) else None
            )
